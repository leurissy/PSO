...
void Solucao::geraSteinerAleatorio( const Ponto& canto_superior, const Ponto& canto_inferior ) {
	...
	// gera pontos de Steiner aleatórios
...
//		cout << Q << endl;
	...
	// limpa a matriz de adjacencia antes do cálculo.
	grafo.limpaDados();
...
Vetor<Ponto> Solucao::passo_de_particula( const Solucao& melhor_solucao_propria,  const Solucao& melhor_solucao_social, double peso, double c1, double c2 ) const {
	...
	// solução com diferentes valores randômicos por dimensão
...
//	{
//		for( int i = 0; i < tamanho; i++ ) {
//			if( i < num_obrigatorios && grafo.grau(i) != 1 || i >= num_obrigatorios && !( grafo.grau(i) == 3 || grafo.grau(i) == 0 ) ) {
//				debugando("funcao Solucao::caminha() - Nao conformidade persistente depois de executar removeNaoConformidade().");
//				throw myexception("funcao Solucao::caminha() - Nao conformidade persistente depois de executar removeNaoConformidade().");
//			}
//		}
//	}
//	// debug
...
//	cout << "grafo_velho depois: " << endl << grafo_velho << endl;
...
//	Vetor<int> novoVetTop = getVetTop();

//	if( this->vetTop == novoVetTop )
//		return custo_por_Smith;

//	this->vetTop = novoVetTop;
//	smith.buildtree(this->vetTop);
...
void Solucao::debugando( string s ) const {
		cout << s << " ========================" << endl;
		cout << *this << endl;
		system("pause");
}
