...
namespace std {
    ...
    Arranjo<Aresta> Grafo::getArestas() const {
        int i;
        list<Aresta> lista;
    ...
    void Grafo::copia_reduzindo(const Grafo& b) {
        ...
        for (int i = 0; i < num_vertices; i++) {
            ...
            //			cout << x << " ";
            //		cout << endl;
            // verificando se h� erros no algoritmo. Pode ser removida na versao definitiva
            for (int x : vertice[i]) {
                if (x >= num_vertices)
                    throw myexception("Erro em Grafo::copia_reduzindo() - vertice fora do limite m�ximo (num_vertices).");
            }
        }
    }
    ...
    list<int> Grafo::percorre(int u, int pai) const {
        list<int> aux;
        aux.push_back(u);
//        cout << "u: " << u << endl;
        for (int i : vertice[u]) {
